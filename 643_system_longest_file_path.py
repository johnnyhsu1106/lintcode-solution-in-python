...
class Solution:
    ...
    def lengthLongestPath(self, input):
        ...
        for line in lines:
            level = self.count_level(line)

            while stack and level <= self.count_level(stack[-1]):
                stack.pop()

            stack.append(line)

            if self.is_file(line):
                max_len = max(self.get_length(stack), max_len)
    ...
    def count_level(self, string):
        return string.count('\t')


    def get_length(self, stack):
        full_path = ''
        for line in stack:
            new_line = line.lstrip('\t')
            full_path += '/' + new_line

        return len(full_path) - 1 # /dir/item..... the first '/' doesn't count
